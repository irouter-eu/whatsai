...
test('MCP reaches the real daemon and cannot override its declared action', {skip:!existsSync(binary)}, async()=>{
 ...
 try {
  ...
  assert.match(tool.description??'',/agent testharness@\S+, private/,'the tool announces which agent this session is and that it is private');
  ...
  assert.equal(JSON.parse((published.content as any[])[0].text).published,true);
