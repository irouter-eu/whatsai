...
test('MCP reaches the real daemon and cannot override its declared action', {skip:!existsSync(binary)}, async()=>{
 ...
 try {
  ...
  assert.equal(v.adapter,'0.9.1');assert.equal(v.plugin,'test-plugin');assert.match(v.daemon,/^\d+\.\d+\.\d+$/);assert.equal(typeof v.database,'number');assert.equal(v.mismatch,v.daemon!=='0.9.1');
