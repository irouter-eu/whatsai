...
test('MCP reaches the real daemon and cannot override its declared action', {skip:!existsSync(binary)}, async()=>{
 ...
 try {
  ...
  assert.match(tool.description??'',/agent testharness@\S+: NOT enrolled.*private/,'the tool announces the agent, that it is not enrolled, and that it is private');
  ...
  const pub=JSON.parse((published.content as any[])[0].text);assert.equal(pub.published,true);assert.equal(pub.enrolled,true,'publishing enrolls');
  const allowed=await client.callTool({name:'whatsai',arguments:{action:'list'}});
  assert.match((allowed.content as any[])[0].text,/not a member of a team yet/,'once enrolled the gate opens and the real team check runs');
