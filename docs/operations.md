...
`whatsai start --name Alice` starts a daemon using `WHATSAI_STATE` (default `~/.local/share/whatsai`). `whatsai health` checks it and `whatsai stop` shuts it down. Logs are in the state directory. Private state and IPC are restricted to the local user; each state directory has a single-process lock. The daemon socket lives inside the state directory, and Unix socket paths are limited to about 100 bytes, so keep `WHATSAI_STATE` short; the daemon and CLI refuse an overlong path with an explicit error rather than failing at bind time. A foreground alternative is `whatsai-daemon --state /absolute/state --name Alice`.
