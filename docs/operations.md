...
- **A session says its agent is not enrolled:** that workspace is not bound to any team. Checkouts matching a team enroll themselves, by Git origin or by the exact directory a path-bound team was created or joined from; anything else needs `whatsai agent enroll LABEL --into WORKSPACE` from your shell, which is deliberate so an unrelated project cannot read team messages or the join key. `whatsai agent unenroll LABEL` closes it again.
- **"pass --team":** the current directory is bound to none of your teams, or you have several; name one by workspace, repository, or id.
