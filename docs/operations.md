...
`whatsai start --name Alice` starts a daemon using `WHATSAI_STATE` (default `~/.local/share/whatsai`). `whatsai health` checks it and `whatsai stop` shuts it down. Logs are in the state directory. Private state and IPC are restricted to the local user; each state directory has a single-process lock. A foreground alternative is `whatsai-daemon --state /absolute/state --name Alice`.
