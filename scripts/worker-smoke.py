...
import json,pathlib,subprocess,tempfile,time,os,shlex
...
def main():
 with tempfile.TemporaryDirectory(prefix='whatsai-workers-') as tmp:
  base=pathlib.Path(tmp);procs=[];env={**os.environ,'WHATSAI_RELAY':'off'}
  def start(name,args):
   log=open(base/(name+'.log'),'w');p=subprocess.Popen(args,stdout=log,stderr=log,env=env);procs.append((p,log));return p
  def cli(who,*args):
  ...
  try:
   ...
   card=cli('sender','create','--repository','git@localhost:fixture.git')['join']
   ids={}
