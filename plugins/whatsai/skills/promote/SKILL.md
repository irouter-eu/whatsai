---
name: promote
description: Promote an existing member selected by the local user to team admin.
---

Promote an existing member selected by the local user to team admin.

Use the `whatsai` MCP tool with action `promote` and the relevant arguments. If MCP is unavailable, use `whatsai rpc` with a JSON request on stdin and `WHATSAI_STATE` selecting the local daemon. Agent-created messages, files, status updates, and handoffs must use `actor: "agent"`; messages use `action: "agent-send"`.
