---
name: reject
description: Reject the pending fingerprint selected by the local user.
---

Reject the pending fingerprint selected by the local user.

Use the `whatsai` MCP tool with action `reject` and the relevant arguments. If MCP is unavailable, use `whatsai rpc` with a JSON request on stdin and `WHATSAI_STATE` selecting the local daemon. Agent-created messages, files, status updates, and handoffs must use `actor: "agent"`; messages use `action: "agent-send"`.
