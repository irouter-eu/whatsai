---
name: register
description: Initialize or show the persistent local WhatsAI identity.
---

Initialize or show the persistent local WhatsAI identity.

Use the `whatsai` MCP tool with action `register` and the relevant arguments. If MCP is unavailable, use `whatsai rpc` with a JSON request on stdin and `WHATSAI_STATE` selecting the local daemon. Agent-created messages, files, status updates, and handoffs must use `actor: "agent"`; messages use `action: "agent-send"`.
